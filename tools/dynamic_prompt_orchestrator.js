...
exports.DynamicPromptOrchestrator = void 0;
exports.normalizeRequest = normalizeRequest;
exports.calculateCausalSimilarity = calculateCausalSimilarity;
exports.selectForcedInjections = selectForcedInjections;
exports.createConstraintCapsule = createConstraintCapsule;
exports.constitutionToCapsule = constitutionToCapsule;
exports.buildICLPrompt = buildICLPrompt;
const ledger_icl_1 = require("../contract/ledger_icl");
const crypto_1 = require("crypto");
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const eventSchemaRuntimeValidator_1 = require("./workflow_runtime/eventSchemaRuntimeValidator");
/**
 * 生のリクエストを正規化
 */
function normalizeRequest(raw_request) {
    // 意図の正規化
    const intent = {
...
// ヘルパー関数
function normalizeIntent(raw_intent) {
...
function extractEntities(params) {
    const entities = {};
    // UUIDパターン
...
function inferActionSignature(raw_request) {
    ...
    normalized_resource = normalized_resource.replace(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi, ':uuid');
    normalized_resource = normalized_resource.replace(/\/\d+/g, '/:id');
    return `${method} ${normalized_resource}`;
}
function inferOperationType(intent) {
    ...
    if (lower.includes('update') || lower.includes('modify'))
        return 'UPDATE';
    if (lower.includes('read') || lower.includes('get'))
        return 'READ';
    if (lower.includes('execute') || lower.includes('run'))
        return 'EXECUTE';
    return 'UNKNOWN';
}
function inferBlastRadius(raw_request) {
...
function inferRiskLevel(raw_request) {
...
function inferRequiredPermissions(raw_request) {
    const permissions = [];
...
function inferCapabilities(raw_request) {
    const capabilities = [];
    const operation_type = inferOperationType(raw_request.intent);
 ...
 */
function calculateCausalSimilarity(current, past, weights = {
    intent: 0.3,
    operation: 0.25,
    constraint: 0.2,
    risk: 0.15,
    ...
    const intent_similarity = computeStringSimilarity(current.intent.intent_normalized, past.intent_normalized);
    // 2. 操作の類似度
    const operation_similarity = computeStringSimilarity(current.action.signature_normalized, past.operation_signature);
    // 3. 制約の類似度（BlastRadiusの一致度）
    const constraint_similarity = current.governance.blast_radius === past.blast_radius ? 1.0 : 0.0;
    // 4. リスクの類似度
    ...
    const risk_similarity = 1.0 - Math.abs(current_risk_idx - past_risk_idx) / risk_levels.length;
    // 5. 結果の類似度（同じRejectionClassか）
    // 初回なので0.5（中立）
    const outcome_similarity = 0.5;
    // 重み付けスコア
    const total_score = weights.intent * intent_similarity +
 ...
 */
function selectForcedInjections(current, all_traces, constitution_rules) {
    const negative_precedents = [];
    const prohibition_rules = [];
    const hlg_interventions = [];
    // 高リスク時は拒否判例を強制注入
    if (current.governance.risk_level === 'high' || current.governance.risk_level === 'critical') {
        ...
        // 禁止ルールを取得
        prohibition_rules.push(...constitution_rules.filter((r) => r.status === 'active' && r.enforcement_level === 'high'));
        // HLG介入事例を取得
        hlg_interventions.push(...all_traces.filter((t) => t.generated_constraints.some((c) => c.requires_hlg_review)).slice(0, 3));
    }
    return {
        negative_precedents,
        prohibition_rules,
 ...
 * Traceを判定式に圧縮
 */
function createConstraintCapsule(traces, enforcement_level) {
    if (traces.length === 0) {
        return {
            capsule_id: (0, crypto_1.randomUUID)(),
            predicate: 'true',
 ...
 */
function constitutionToCapsule(rule) {
    return {
 ...
 * ICLプロンプトを構築
 */
function buildICLPrompt(context) {
...
class DynamicPromptOrchestrator {
    ledger_store;
    constitution_store;
    constructor(ledger_store, constitution_store) {
        this.ledger_store = ledger_store;
        this.constitution_store = constitution_store;
    }
    /**
     * メインのオーケストレーションフロー
    ...
    async orchestrate(raw_request) {
        ...
        const filtered_traces = (0, ledger_icl_1.applyHardFilter)(this.ledger_store, criteria, {
        ...
        const allowed_outputs = [
            ledger_icl_1.ICLOutputType.TRANSITION_REQUEST,
            ledger_icl_1.ICLOutputType.REQUEST_EVIDENCE,
            ledger_icl_1.ICLOutputType.ESCALATE_HLG,
            ledger_icl_1.ICLOutputType.DENY,
            ledger_icl_1.ICLOutputType.CONDITIONAL_APPROVAL,
        ];
        ...
        const dispatch_record = this.buildDispatchRecordEvent(raw_request.context, normalized, {
            filtered_count: filtered_traces.length,
            injected_count: injected_capsules.length,
            allowed_outputs,
        });
        const dispatch_validation = this.validateAndAuditDispatchRecord(dispatch_record);
        return {
            normalized,
            filtered_traces,
            scored_traces,
    ...
    buildDispatchRecordEvent(raw_context, normalized, meta) {
    ...
    extractDispatchContext(raw_context) {
        ...
        const obj = raw_context;
        const required = ['week_id', 'trace_id', 'correlation_id', 'run_id'];
        const out = {};
    ...
    validateAndAuditDispatchRecord(event_payload) {
        const validation = (0, eventSchemaRuntimeValidator_1.validateWorkflowEvent)(event_payload);
        if (!validation.ok) {
        ...
        const audit_line = {
            ...
            workflow_id: event_payload.workflow_id,
            stage_id: event_payload.stage_id,
    ...
    appendDispatchAuditLog(audit_line) {
        const repo_root = node_path_1.default.resolve(__dirname, '..');
        const log_dir = node_path_1.default.join(repo_root, 'logs');
        const log_path = node_path_1.default.join(log_dir, 'dynamic_prompt_orchestrator.dispatch.audit.jsonl');
        if (!node_fs_1.default.existsSync(log_dir)) {
            node_fs_1.default.mkdirSync(log_dir, { recursive: true });
        }
        node_fs_1.default.appendFileSync(log_path, `${JSON.stringify(audit_line)}\n`, 'utf8');
        return log_path;
    }
}
exports.DynamicPromptOrchestrator = DynamicPromptOrchestrator;
