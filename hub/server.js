...
const proof_1 = require("../contract/proof");
const api_schema_1 = require("../contract/api_schema");
const council_aggregator_1 = require("./council_aggregator");
const append_log_1 = require("./append_log");
const gemini_peripheral_1 = require("../peripherals/gemini_peripheral");
const ollama_peripheral_1 = require("../peripherals/ollama_peripheral");
const vertex_peripheral_1 = require("../peripherals/vertex_peripheral");
const budget_1 = require("../contract/budget");
const ws_1 = require("ws");
const runbookService_1 = require("./runbookService");
const auditService_1 = require("./auditService");
const mockStream_1 = require("./mockStream");
const LOG_FILE_PATH = node_path_1.default.resolve(__dirname, '../../antigravity.log');
const HTTP_PORT = 7777;
const AGGREGATOR_ID = 'central-99';
let this_tick_counter = 0;
void append_log_1.Hub.initialize().catch((error) => {
    console.error('[Hub] Failed to initialize log file:', error);
});
async function appendQuarantineEvent(payload) {
    const lastEvent = await append_log_1.Hub.getLastEvent();
    ...
    const event = (0, proof_1.signEvent)(payloadWithObserved, prevHash, 'L1', payload.aggregator_id);
    event.actor = {
        layer: 'hub',
        name: payload.aggregator_id,
    };
    await append_log_1.Hub.appendEvent(event);
}
// Council Aggregator インスタンス（v0.1: Node A/B を監視）
const councilAggregator = new council_aggregator_1.CouncilAggregator({
    aggregatorId: AGGREGATOR_ID,
    emit: async (payload) => {
        await appendQuarantineEvent(payload);
    },
});
councilAggregator.registerNode('A', 'http://localhost:8001');
councilAggregator.registerNode('B', 'http://localhost:8002');
// Peripherals
const gemini = new gemini_peripheral_1.GeminiPeripheral();
const ollama = new ollama_peripheral_1.OllamaPeripheral();
const vertex = new vertex_peripheral_1.VertexPeripheral();
...
const CONFIG_PATH = node_path_1.default.resolve(__dirname, '../config/runtime.yaml');
// runtime.yaml の変更監視
node_fs_1.default.watch(CONFIG_PATH, async (event) => {
    if (event === 'change') {
        console.log('[GOVERNANCE] runtime.yaml changed. Reloading policy...');
        config_1.globalConfig.reload(); // 仮定: globalConfig にリロードメソッドがあると想定。なければ手動で読み込み。
        const newHash = config_1.globalConfig.getPolicyHash();
        const lastEvent = await append_log_1.Hub.getLastEvent();
        const prevHash = lastEvent?.event_hash || '';
        const updateEvent = (0, proof_1.signEvent)({
            type: 'POLICY_LIVE_UPDATE',
            policy_hash: newHash,
            reason: 'MANUAL_CONFIG_EDIT'
        }, prevHash, 'L2', 'governance-watchdog');
        await append_log_1.Hub.appendEvent(updateEvent);
        console.log(`[GOVERNANCE] Policy Updated: ${newHash.substring(0, 8)}`);
        // SSE で通知
        broadcaster.broadcast(updateEvent);
    }
});
const CONFIG = config_1.globalConfig.getRuntimeConfig();
...
setInterval(async () => {
    try {
        ...
        if (stall_counter >= CONFIG.monitoring.stall_watchdog_threshold) {
            ...
            const recoveryLastEvent = await append_log_1.Hub.getLastEvent();
            const recoveryPrevHash = recoveryLastEvent?.event_hash || '';
            const recoveryEvent = (0, proof_1.signEvent)(recoveryPayload, recoveryPrevHash, 'L1', `hub-${AGGREGATOR_ID}`);
            recoveryEvent.actor = { layer: 'hub', name: 'watchdog' };
            await append_log_1.Hub.appendEvent(recoveryEvent);
        ...
        const heartbeatPayload = {
            ...
            build_id: config_1.globalConfig.getVersion().split('\n')[0].split(': ')[1],
            metrics: {
        ...
        const lastEvent = await append_log_1.Hub.getLastEvent();
        const prevHash = lastEvent?.event_hash || '';
        const event = (0, proof_1.signEvent)(heartbeatPayload, prevHash, 'L1', `hub-${AGGREGATOR_ID}`);
...
function readAllLogs() {
    try {
        const content = node_fs_1.default.readFileSync(LOG_FILE_PATH, 'utf-8');
        const lines = content
            .trim()
            .split('\n')
 ...
 */
function enrichEventMetadata(events) {
    ...
    events.forEach((event) => {
        const targetId = event.payload.correction_of;
        if (targetId) {
    ...
    const adoptedCorrectionByTarget = new Map();
    ...
    return events.map((event) => {
        const isCorrection = !!event.payload.correction_of;
        const correctsEventId = event.payload.correction_of;
        // corrected_by: このイベントを直接指す全訂正イベント
        const correctionEvents = correctionsByTarget.get(event.event_id) || [];
        const correctedByIds = correctionEvents.map((c) => c.event_id);
...
function calculateDiagnostics(enrichedEvents) {
    // Clear stats
    ...
    enrichedEvents.forEach(event => {
        ...
        if (event.event_type === 'REJECTION' || (event.payload && event.payload.type === 'REJECTION')) {
            ...
            if (reason.includes('budget'))
                rejection_counts.budget++;
            else if (reason.includes('safety'))
                rejection_counts.safety++;
            else if (reason.includes('sincerity'))
                rejection_counts.sincerity++;
            else if (reason.includes('logic'))
                rejection_counts.logic++;
            else
...
class SSEBroadcaster {
    ...
    startWatching() {
        ...
        node_fs_1.default.watchFile(LOG_FILE_PATH, (curr, prev) => {
    ...
    stop() {
        node_fs_1.default.unwatchFile(LOG_FILE_PATH);
 ...
 */
function getEventsByCursor(query) {
    const events = readAllLogs();
...
function getCouncilState() {
    ...
    const forks = councilLogs
        .filter((e) => e.fork_count && e.fork_count > 1)
        .map((e) => ({
    ...
    // 最新の採用訂正
    const adoptedCorrections = councilLogs.filter((e) => e.is_adopted);
 ...
 */
function getCouncilHistory(query) {
    const events = readAllLogs();
...
wss.on('connection', (ws) => {
    ...
    let isNoisy = false;
    // Start mock stream
    (0, mockStream_1.mockLiveApiStream)(ws, () => isNoisy);
    ws.on('message', async (message) => {
        try {
            ...
            switch (action) {
                case 'INIT_ROLLBACK':
                    await auditService_1.auditService.logEvent(ws, 'SECURITY', 'AUTHORIZE_EXECUTION', 'Gated authorization verified. Triggering rollback.');
                    ...
                    for (let i = 0; i <= 100; i += 25) {
                        setTimeout(() => {
                            ...
                            if (i === 100) {
                                auditService_1.auditService.logEvent(ws, 'SYSTEM', 'ROLLBACK_COMPLETE', 'V1.2 restored successfully.');
                            }
                        }, i * 50);
                    }
                    break;
                case 'OPEN_2FA':
                    const runbook = runbookService_1.runbookService.getRunbook('DEPLOY_FAIL');
                    ws.send(JSON.stringify({ type: 'RUNBOOK_DATA', data: runbook }));
                    await auditService_1.auditService.logEvent(ws, 'OPERATIONAL', 'RUNBOOK_FETCH', 'Evidence-based runbook synthesized for DEPLOY_FAIL');
                    break;
                default:
                    await auditService_1.auditService.logEvent(ws, 'SVP', action, 'Command processed');
            }
        }
        catch (e) {
...
const server = node_http_1.default.createServer((req, res) => {
    const urlObj = new node_url_1.URL(req.url || '/', `http://${req.headers.host || 'localhost'}`);
    ...
    if (req.url?.startsWith('/api/events') && req.method === 'GET') {
        try {
            ...
            const validation = (0, api_schema_1.validateRequest)(api_schema_1.EventsQuerySchema, queryParams);
    ...
    if (pathName === '/api/governance/quarantine' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                const lastEvent = await append_log_1.Hub.getLastEvent();
                const prevHash = lastEvent?.event_hash || '';
                const interventionEvent = (0, proof_1.signEvent)({
                    type: 'QUARANTINE_NODE',
                    node_id: nodeId,
                    reason: 'UI_INTERVENTION',
                    aggregator_id: AGGREGATOR_ID
                }, prevHash, 'L2', 'human-governor');
                await append_log_1.Hub.appendEvent(interventionEvent);
    ...
    if (pathName === '/api/diagnostics' && req.method === 'GET') {
        try {
            ...
            const stats = {
                ...
                policy_hash: config_1.globalConfig.getPolicyHash()
            };
            res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (req.url?.startsWith('/api/council/history') && req.method === 'GET') {
        try {
            ...
            const validation = (0, api_schema_1.validateRequest)(api_schema_1.CouncilHistoryQuerySchema, queryParams);
            if (!validation.success) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ error: validation.error }));
                return;
    ...
    if (pathName === '/api/generate' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                    ...
                    if (selectedProvider === 'gemini') {
                        ...
                        for await (const chunk of stream) {
                            if (chunk.done) {
                                ...
                                vertexUsage.grounding_metadata = chunk.result.grounding_metadata;
                            }
                            else {
                                ...
                                broadcaster.broadcast({
                                    ...
                                    budget_snapshot: budget_1.globalBudgetTracker.snapshot()
                                });
                            }
                        }
                    }
                ...
                const lastEvent = await append_log_1.Hub.getLastEvent();
                const prevHash = lastEvent?.event_hash || '';
                const event = (0, proof_1.signEvent)({
                    type: 'AI_GENERATION',
                    prompt,
                    response: responseText,
                    sincerity_score: sincerityScore,
                    selection_status: 'PASS'
                }, prevHash, targetTier, `hub-${AGGREGATOR_ID}`);
...
async function commitInitialPolicy() {
    ...
    const policyPayload = {
        type: 'POLICY_UPDATE',
        policy_hash: config_1.globalConfig.getPolicyHash(),
        config: config_1.globalConfig.getRuntimeConfig(),
        reason: 'GENESIS_BOOT'
    };
    const event = (0, proof_1.signEvent)(policyPayload, prevHash, 'L2', 'governance');
    await append_log_1.Hub.appendEvent(event);
    console.log(`[GOVERNANCE] Policy Commit: ${config_1.globalConfig.getPolicyHash().substring(0, 8)} (Tier: L2)`);
}
// Upgrade HTTP to WebSocket for /ws/live
server.on('upgrade', (request, socket, head) => {
    const pathname = new node_url_1.URL(request.url || '', `http://${request.headers.host}`).pathname;
    if (pathname === '/ws/live') {
...
async function startServer() {
    await append_log_1.Hub.initialize();
